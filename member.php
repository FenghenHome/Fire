	...
	if ($action == 'login') {
		...
		if ($token) {
			if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['tradeno'])) {
				...
				$data = json_decode(file_get_contents('http://pay.114rom.com/api/pay.php?application=2&apikey=W6sz7qFxcfoudkeplxdPDqptWhBVUZCPrcgIzPb5wv8avfrx3vJFlhdxsqlqbZF9&method=check_order&update=1&tradeNo='.$tradeno), true);
				if (isset($data['error'])) {
					// 表明正确获取
					if (!$data['error']) {
